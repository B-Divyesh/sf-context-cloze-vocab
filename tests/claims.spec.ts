...
test('@claim:json-export exports all demo words and reviews', async ({ page }) => {
  await page.goto('/demo');
  ...
  const imported = {
    ...
    items: [{ ...data.items[0], id: 'import-check', word: 'resilient', sentence: 'The resilient bridge remained open.' }],
    reviews: []
  };
...
test('@claim:offline-reload demo reloads offline after the first visit', async ({ page, context }) => {
  ...
  await page.waitForFunction(async () => Boolean(await caches.match('/assets/app.js')));
...
test('@claim:paid-license a returned license unlocks the paid view', async ({ page }) => {
